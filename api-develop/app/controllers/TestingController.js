...
class TestingController {
        ...
        try {
            ...
            if (req.user.user_group !== appConfig.USER_GROUP.STUDENT) {
                if (req.user.user_group === appConfig.USER_GROUP.TEACHER || req.user.user_group === appConfig.USER_GROUP.SUPPORTER) {
                    subjectIds = req.user.subject_ids;
                } else {
                    subjectIds.push(subjectID);
                }

                if (subjectIds.length === 0)
                    return response(res, null, 'Vui lòng chọn môn học trước!', statusCode.ERROR);
            ...
            if (keyword) {
                conditions.$or = [
                    { 'exam.code': { $regex: keyword, $options: 'i' } },
                    { 'user.name': { $regex: keyword, $options: 'i' } },
                    { 'classroom.name': { $regex: keyword, $options: 'i' } },
                    { 'subject.name': { $regex: keyword, $options: 'id' } }
                ];
            }
            ...
            let chapterIds = [];
            let categories = [];
            let classroomExamIds = [];
            let total = 0;
            let classroomTestingExamIds = [];
            if (req.user.user_group == appConfig.USER_GROUP.STUDENT) {
                if (!classroomID || !status)
                    return response(res, null, 'Yêu cầu không hợp lệ!', statusCode.ERROR);

                const userTesting = await UserTestingModel.findOne({ user_id: req.user.user_id });

                conditions = {
                    'classroom.id': classroomID
                };
                if (status === 'PENDING') {
                    const projection = '_id exam_id type exam subject classroom status is_fixed_time started_at finished_at';


                    chapters = await ChapterClassroomModel.find({ classroom_id: classroomID });
                    chapterIds = [];
                    for (let i = 0; i < chapters.length; i++) {
                        const chapterID = chapters[i].chapter ? chapters[i].chapter.id : null;
                        if (chapterID)
                            chapterIds.push(chapterID);
                    }

                    categories = await CategoryModel.find({ "chapter.id": { $in: chapterIds } });
                    classroomExamIds = [];
                    for (let i = 0; i < categories.length; i++) {
                        const _category = categories[i];
                        const examId = Array.isArray(_category.exam) ? _category.exam[0]?.id : _category.exam?.id;
                        if (examId)
                            classroomExamIds.push(examId);
                    }


                    // Tim ra nhung exam ids cua Lop X da lam
                    if (userTesting && userTesting.exam_ids && userTesting.exam_ids.length > 0) {
                        for (let i = 0; i < userTesting.exam_ids.length; i++) {
                            if (classroomExamIds.indexOf(userTesting.exam_ids[i]) >= 0) {
                                classroomTestingExamIds.push(userTesting.exam_ids[i]);
                            }
                        }
                    }
                    ...
                    if (userTesting) {
                        if (userTesting.exam_ids && userTesting.exam_ids.length > 0) {
                            conditions['exam.id'] = {
                                $nin: userTesting.exam_ids
                            };
                        }
                    }
                    // conditions.status = { $in: ['SENT', 'DONE'] };
                    conditions.status = 'PENDING';
                    conditions['classroom.id'] = classroomID;
                    records = await ExamClassroomModel.find(conditions, projection, options);
                    total = await ExamClassroomModel.count(conditions);
                    for (let i = 0; i < records.length; i++) {
                        if (records[i].exam && records[i].exam.id) {
                            if (examIds.indexOf(records[i].exam.id) < 0)
                                examIds.push(records[i].exam.id);
                        }
                    }

                    if (examIds.length <= 0) {
                        if (userTesting && userTesting.exam_ids)
                            examData = await ExamModel.find({ _id: { $nin: userTesting.exam_ids }, _id: { $in: _examIds } }, { doc_link: 1, video_link: 1 });
                    } else {
                        examData = await ExamModel.find({ _id: { $in: examIds } }, { doc_link: 1, video_link: 1 });
                    }
                }
                ...
                if (status === 'DONE') {
                    ...
                    conditions.deleted_at = null;
                    records = await TestingModel.find(conditions, null, options);
                    total = await TestingModel.count(conditions);
                    examIds = [];
                    for (let i = 0; i < records.length; i++) {
                        if (records[i].exam && records[i].exam.id) {
                            if (examIds.indexOf(records[i].exam.id) < 0)
    ...
    async updatePoint(req, res, params) {
        try {
            const id = params.id || null;
            let point = params.point || 0;
            ...
            if (!id || !point)
                return response(res, null, 'Request không hợp lệ!', statusCode.ERROR);
            const rs = await TestingModel.updateOne({ _id: id }, { $set: { point: point } });
            if (rs.nModified)
    ...
    async delete(req, res, params) {
        try {
            ...
            const rs = await TestingModel.softDelete({ _id: { $in: ids } }, true);

            if (rs) {
                const conditions = {};
                conditions['testing.id'] = { $in: ids };
                await PointLogModel.delete(conditions, true);
                AppService.removeExamIDFromTesting(ids);
                return response(res, {}, 'Thành công', statusCode.OK);
            }
