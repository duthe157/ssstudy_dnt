...
class ModalAttachedBook extends Component {
    constructor(props) {
        if (props.classroom)
            console.log(props)
        super();
    ...
    handleDeleteBook = async(id) => {
        let { book_attached, _id } = this.props.classroom;

        if (book_attached && isArray(book_attached)) {
            const new_arr = book_attached.filter(item => item != id);

            const params = {
                book_attached: new_arr,
                classroom_id: _id
            };
    ...
    UNSAFE_componentWillReceiveProps(nextProps) {
        ...
        if (this.props.bookAttached !== nextProps.bookAttached) {
            this.setState({
                bookAttacheds: nextProps.bookAttached,
                selectedBook: ''
            })
        }
    }
    ...
    handleSubmit = async () => {
        ...
        if (selectedBook && selectedBook != '') {
            if (book_attached && !book_attached.includes(selectedBook)) {
                book_attached.push(selectedBook);
                const params = {
                    book_attached: book_attached,
                    classroom_id: _id
                };
...
export default withRouter(
    connect(mapStateToProps, mapDispatchToProps)(ModalAttachedBook)
);
