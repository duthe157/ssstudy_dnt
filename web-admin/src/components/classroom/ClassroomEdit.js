...
import { showClassroom, updateClassroom, removeChapter, addChapter, updateGroupChapter } from "../../redux/classroom/action";
...
class ClassroomEdit extends Component {
  ...
  async componentDidMount() {
    ...
    if (this.props.classroom) {
      ...
      if (this.props.classroom.includes && this.props.classroom.includes.length > 0) {
        ...
        updatedIncludesData = includes.slice(4).map((item, index) => {
          const getIconKeyFromIndex = (iconIndex) => {
            const iconKeyMap = ['Book', 'BookOpen', 'FileText', 'Clock', 'CheckSquare', 'ListChecks', 'PlusSquare', 'Play', 'Menu', 'Image', 'Video', 'Link', 'BarChart', 'Info', 'Star', 'Users', 'FolderPlus', 'Layers'];
          ...
          return {
            ...
            text: item.text || '',
            mainIcon: item.iconKey ? this.getIconComponentFromKey(item.iconKey) : ICON_LIST[item.icon || 0].component,
            icon: item.icon || 0,
      ...
      if (this.props.classroom.group_chapter && Array.isArray(this.props.classroom.group_chapter)) {
        // Đảm bảo mỗi group có orderno, nếu không có thì gán theo index
        initialGroups = this.props.classroom.group_chapter.map((group, index) => ({
          ...group,
      ...
      this.setState({
        ...
        selectedClassroomAttachedIDs: classroom_attached ? classroom_attached.map(item =>
          typeof item === 'string' ? item : (item.id || item._id)
        ) : [],
        listClassroomAttacheds: this.props.classroomAttached || classroom_attached || [],
        group: initialGroups,
  ...
  handleChaptersUpdate = async (newChapters) => {
    ...
    const fallbackGroupId = activeGroupId || (currentGroups[0]?.id ?? null) || chapters[0]?.group_id || null;
    if ((fallbackGroupId === null && chapters.length > 0) || (currentGroups.length === 0)) {
      const uniqueSubjectMap = new Map();
      let hasChaptersWithoutSubject = false;

      chapters.forEach(ch => {
        if (ch.selected_subject_id && !uniqueSubjectMap.has(ch.selected_subject_id)) {
          const matchingSubject = this.props.subjects && this.props.subjects.find(subject => subject._id === ch.selected_subject_id);
          if (matchingSubject) {
      ...
      if (uniqueSubjectMap.size > 0 || hasChaptersWithoutSubject) {
        let groupId = 1;
        let orderNo = 1;
        const newGroups = Array.from(uniqueSubjectMap.entries()).map(([subjectId, subjectName]) => ({
          id: groupId++,
          title: subjectName,
  ...
  handleSubmit = async (e) => {
    ...
    if (selectedChapters && selectedChapters.length > 0) {
      selectedChapters.map((item, index) => {
        ...
        // Thêm group.title nếu có
        const matchingGroup = this.state.group.find(g => g.id === item.group_id);
        if (matchingGroup) {
          chapterData.group_title = matchingGroup.title;
        }
  ...
  deleteGroup = async (groupId) => {
    ...
    const chaptersToRemove = selectedChapters.filter((chapter) => chapter.group_id === groupId);
    if (chaptersToRemove.length > 0) {
      await Promise.all(chaptersToRemove.map((chapter) => this.props.removeChapter({
          ...
          >
            {ICON_LIST.map((item, index) => (
              <button
                ...
                onClick={() => this.handleIconSelectForEdit(item.component, item.index)}
              >
                {item.component}
              </button>
            ))}
  ...
  handleAddChapter = async (data, groupID) => {
    ...
    if (activeGroupId) {
      ...
      const existingGroup = this.state.group.find(g => g.id === activeGroupId);
      if (existingGroup) {
  ...
  render() {
    ...
    return (
      <div>
                ...
                <div className="block-content">
                  <div className="item-input-text">
                    ...
                    <div className="form-group mr-16" style={{ width: "20%" }}>
                      ...
                      <DatePicker
                        format={"YYYY/MM/DD"}
                        value={timeCourse.closing_date ? moment(timeCourse.closing_date) : null}
        ...
        <ModalAttachedBook handleAddBook={this.handleAddBookAttached}
          handleAddSelectedBookAttched={this.handleAddSelectedBookAttched}
          selectedBookAttachedIDS={this.state.selectedBookAttachedIDS}
          selectedBooks={this.state.listBookAttacheds} />
        <ModalBookRelate
