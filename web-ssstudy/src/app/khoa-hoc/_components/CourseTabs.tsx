...
import ReviewSection from "@app/khoa-hoc/_components/EvalueteSourse";
...
export default function CourseTabs({
  ...
  }, [activeGroupTab, subjects, checkScrollPosition]);
  ...
  const getChapterCompletion = (chapter: Chapter) => {
    ...
    const completed = chapter.category.filter(
      (cat) => cat.is_done_video || cat.is_done_exam
    ).length;
  ...
  const mainTabs = [
    ...
    { id: "evaluate", label: "Đánh giá", count: null },
  ];
    ...
    <div className="book-tabs">
      ...
      <div className="tabs-content">
                                ...
                                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                                  ...
                                  <div className="flex items-start space-x-3 flex-1 min-w-0">
                                    ...
                                    <div className="min-w-0 flex-1">
                                      <div
                                        onClick={() => {
                                          ...
                                          if (!isLoggedIn) {
                                            ...
                                            const currentPath = `/khoa-hoc/${courseId}`;
                                            router.push(
                                          ...
                                          if (
                                            !category.is_free &&
                                            !courseData.isJoined && authService.getCurrentUser()?.user_group !== 'ADMIN'
                                          ) {
                                            toast.info(
                                      ...
                                      >
                                        <h6 className="text-sm font-medium text-blue-600 hover:text-blue-800 cursor-pointer whitespace-normal break-words">
                                          {highlightSearchTerm(
                                            category.name,
                                            searchTerm
                                          )}
                                        ...
                                        <div className="flex items-center gap-2 md:hidden">
                                          {isLive ? (
                                            <>
                                              ...
                                              {action && (
                                                <button
                                                  className={`px-3 py-1.5 rounded text-xs font-semibold ${action.className}`}
                                                  onClick={() =>
                                                    action.action?.()
                                                  }
                                                >
                                  ...
                                  <div className="hidden md:flex items-center space-x-3 flex-shrink-0">
                                    {isLive ? (
                                      <>
                                        {action && (
                                          <button
                                            className={`px-4 py-2 rounded text-sm font-semibold ${action.className}`}
        ...
        {activeMainTab === "evaluate" && (
          <div className="tab-panel active">
            <ReviewSection classroom_id={courseId} />
          </div>
        )}
      </div>
    </div>
