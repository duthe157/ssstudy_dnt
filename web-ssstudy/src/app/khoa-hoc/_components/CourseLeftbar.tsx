...
interface CourseLeftbarProps {
  onClose?: () => void;
  basePath?: string; // Base path for navigation (e.g., '/khoa-hoc' or '/tim-kiem')
}
...
/**
 * Component hiển thị sidebar bên trái của trang khóa học
 */
export default function CourseLeftbar({
  onClose,
  basePath = "/khoa-hoc",
}: CourseLeftbarProps) {
  ...
  const handleSubmenuClick = (course: Course) => {
    ...
    // If we're on the search page and type is not set, set it to CLASSROOM
    if (basePath === "/tim-kiem" && !params.get("type")) {
      params.set("type", "CLASSROOM");
  ...
  return (
    <div className="bg-white rounded-lg shadow-md h-full flex flex-col">
      ...
      <div className="lg:hidden flex items-center justify-between p-4 border-b">
        <h2 className="text-lg font-semibold">Lọc khóa học</h2>
        <button
          onClick={onClose}
                    ...
                    {category.courses.map((course) => {
                      ...
                      return (
                        <div
                          ...
                          role="button"
                          tabIndex={0}
                          aria-label={`Tìm kiếm khóa học ${course.name}`}
