...
export default function BookDetailClient({ alias }: BookDetailClientProps) {
  ...
  const [extras, setExtras] = useState<{
    ...
    teacher?: any[];
  }>({ classroomAttached: [], bookRelates: [], bookAttached: [] });
  ...
  useEffect(() => {
    const fetchBook = async () => {
      try {
        ...
        const resp = await bookidService.getBookDetail(alias);
        const root = (resp as any)?.data ?? null;
        const data = root?.data?.book ?? root?.book ?? null;
        if (!data) {
          setError("Không tìm thấy sách id");
          return;
        }
        const mapped: Book = {
          ...
          bookId_attached: data.bookId_attached || [],
          combo_mode: Boolean(data.combo_mode),
          student_owned:
        ...
        const bookAttachedFromRoot = root?.data?.bookIdAttached ?? root?.data?.bookAttached ?? root?.data?.bookId_attached ?? [];
        let finalClassroomAttached: any[] = root?.data?.classroomAttached ?? [];

        if (mapped.combo_mode && mapped.bookId_attached && mapped.bookId_attached.length > 0) {
          try {
            // Lấy danh sách ID sách đính kèm
            const attachedBookIds = typeof mapped.bookId_attached[0] === "string" 
              ? (mapped.bookId_attached as string[])
              : (mapped.bookId_attached as any[]).map(b => b._id || b.id).filter(id => !!id);

            // Fetch chi tiết từng sách để lấy classroom_attached của chúng
            const bookFetchPromises = attachedBookIds.map(async (id) => {
              try {
                const bResp = await bookidService.getBookDetail(id);
                const bRoot = (bResp as any)?.data ?? null;
                // Lấy classroomAttached từ root (đối tượng đầy đủ) hoặc từ book (IDs)
                const attachedCourses = bRoot?.data?.classroomAttached ?? bRoot?.classroomAttached ?? [];
                // Tag courses with the source book ID they belong to
                return attachedCourses.map((c: any) => ({ ...c, _from_book_id: id }));
              } catch (e) {
                console.error(`Error fetching courses from attached book ${id}:`, e);
                return [];
              }
            });

            const courseArrays = await Promise.all(bookFetchPromises);
            // Hợp nhất và loại bỏ trùng lặp theo _id/id
            const allCourses = courseArrays.flat();
            const uniqueCoursesMap = new Map();
            allCourses.forEach(c => {
              const cid = c?._id || c?.id;
              if (cid && !uniqueCoursesMap.has(cid)) {
                uniqueCoursesMap.set(cid, c);
              }
            });
            finalClassroomAttached = Array.from(uniqueCoursesMap.values());
          } catch (err) {
            console.error("Error processing courses for combo mode:", err);
          }
        }

        const nextExtras = {
          classroomAttached: finalClassroomAttached,
          bookRelates: root?.data?.bookRelates ?? [],
          bookAttached: [], // Không hiển thị sách nữa
        ...
        if (mapped.level && mapped.category?.id) {
          try {
            const relatedResponse = await bookidService.getRelatedBooks({
              page: 1,
              limit: 20,
              level: mapped.level,
      ...
      <div className="max-w-6xl mx-auto">
        <div className="bg-white rounded-lg shadow-sm border p-8 text-center">
          <div className="text-red-600 text-lg mb-4">
            {error || "Không tìm thấy sách id"}
          </div>
          <Link href="/sach-id" className="text-blue-600 hover:text-blue-800">
            ← Quay lại danh sách sách id
          </Link>
        </div>
      </div>
    );
  ...
  const detail: BookDetailData = {
    ...
    comboMode: book.combo_mode,
    // Khóa học đi kèm từ classroomAttached API
    relatedCourses: (extras.classroomAttached || []).map((c: any) => {
      const courseId = c?._id || c?.id || "";
      const courseImage = c?.image
        ? c.image.startsWith("http")
          ? c.image
          : `${CDN_LINK}${c.image}`
        : book.image || "/imgs/logo.png";
      return {
        id: courseId,
        name: c?.name || "",
        thumbnail: courseImage,
        url: c._from_book_id 
          ? `/sach-id/${c._from_book_id}/${courseId}`
          : `/sach-id/${alias}/${courseId}`,
      };
    }),
    stats: {
    ...
    previewLink: book.demo_link || book.external_link,
  };
  ...
  const giftItems = (extras.classroomAttached || []).map((c: any) => ({
    ...
    name: c?.name || "",
    thumbnail: c?.image 
      ? (c.image.startsWith("http") ? c.image : `${CDN_LINK}${c.image}`)
      : "/imgs/logo.png",
    url: c._from_book_id
      ? (c?.alias ? `/sach-id/${c._from_book_id}/${c.alias}` : c?._id ? `/sach-id/${c._from_book_id}/${c._id}` : "#")
      : (c?.alias ? `/sach-id/${alias}/${c.alias}` : c?._id ? `/sach-id/${alias}/${c._id}` : "#"),
    href: c._from_book_id
      ? (c?.alias ? `/sach-id/${c._from_book_id}/${c.alias}` : c?._id ? `/sach-id/${c._from_book_id}/${c._id}` : "#")
      : (c?.alias ? `/sach-id/${alias}/${c.alias}` : c?._id ? `/sach-id/${alias}/${c._id}` : "#"),
